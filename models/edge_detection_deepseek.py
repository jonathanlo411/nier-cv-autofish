...
class BobDetector:
        ...
        if edge_count > 0:
            ...
            if len(edge_points) > 1:
                ...
                if features['edge_std_y'] > 0 and features['edge_std_x'] > 0:
                    features['edge_density'] = edge_count / \
                        (features['edge_std_y'] * features['edge_std_x'])
                else:
                ...
                if np.mean(horizontal_profile) > 0:
                    features['horizontal_continuity'] = np.max(
                        horizontal_profile) / np.mean(horizontal_profile)
                else:
    ...
    def update(self, edges_frame):
        ...
        self.edge_mad = np.median(
            np.abs(recent_edges - self.baseline_edge)) * 1.4826

        self.baseline_density = np.median(recent_densities)
        self.density_mad = np.median(
            np.abs(recent_densities - self.baseline_density)) * 1.4826

        # Calculate z-scores
        if self.edge_mad > 0:
            self.edge_zscore = (
                edge_count - self.baseline_edge) / self.edge_mad
        else:
            self.edge_zscore = 0

        if self.density_mad > 0:
            self.density_zscore = (
                density - self.baseline_density) / self.density_mad
        else:
        ...
        if features['horizontal_continuity'] > 2.0:
            confidence += min((features['horizontal_continuity'] -
                              2.0) / 4.0, 1.0) * 0.3
...
def process_video(path):
    ...
    while True:
        ...
        if state == "idle":
            ...
            if established and absent_streak >= min_absent_frames and ml_confidence > 0.3:
                ...
                print(
                    f"[ml_edge_detection] BITE at t={time_s:.2f}s "
        ...
        csv_writer.writerow([frame_idx,
                             ...
                             established,
                             state,
                             detector.edge_zscore,
                             detector.density_zscore,
                             ml_confidence])
        t_all.append(time_s)
        ...
        cv2.putText(
            vis,
            f"edge_z={detector.edge_zscore:.1f} dense_z={detector.density_zscore:.1f}",
            (10,
             55),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
    ...
    with open(os.path.join(outdir, "log.txt"), "w") as f:
        ...
        f.write(
            f"ml_params: window_size=2.0s, edge_weight=0.4, density_weight=0.3, continuity_weight=0.3\n\n")
        if not triggers:
            f.write("No bites detected.\n")
        for ts, absent, conf in triggers:
            f.write(
                f"t={ts:.2f}s  absent_frames={absent}  ml_confidence={conf:.2f}\n")
