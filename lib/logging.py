...
class FishingLogger:
    ...
    def _write_header(self):
        ...
        with open(self.filepath, 'w') as f:
            ...
            f.write("-" * 80 + "\n")
            f.write(f"{'Cycle':<8} {'Status':<12} {'Catch Time(s)':<16} {'Avg FG':<12} {'Avg Z':<12} {'Trigger Z':<12}\n")
            f.write("-" * 80 + "\n")
    ...
    def _finalize_cycle(self):
        ...
        avg_fg = statistics.mean(self.cycle_fg_values) if self.cycle_fg_values else 0
        avg_z = statistics.mean(self.cycle_z_values) if self.cycle_z_values else 0
...
class FishingLogger:
    ...
    def write_summary(self):
        ...
        avg_catch_time = (self.total_fishing_time / valid_catches) if valid_catches > 0 else 0
        avg_z = statistics.mean(self.all_z_values) if self.all_z_values else 0
        avg_fg = statistics.mean(self.all_fg_values) if self.all_fg_values else 0
        timeout_rate = (self.total_timeouts / self.total_catches * 100) if self.total_catches > 0 else 0
        ...
        with open(self.filepath, 'w') as f:
            ...
            f.write(f"  Total Session Time:  {total_session_time:.1f}s ({total_session_time/60:.1f} min)\n")
            ...
            f.write(f"  Avg Time per Catch:  {avg_catch_time:.1f}s (successful only)\n")
            ...
            # Config details
            cfg = self.config
            f.write(f"  Monitor:             {cfg.get('monitor_width')}x{cfg.get('monitor_height')} at ({cfg.get('monitor_left')},{cfg.get('monitor_top')})\n")
            ...
            f.write(f"  Baseline Window Sec: {cfg.get('baseline_window_sec')}\n")
            ...
            f.write(f"  Look-down Duration:  {cfg.get('look_down_duration')}s\n")
            f.write(f"  Look-down Speed:     {cfg.get('look_down_speed')}\n")
            f.write(f"  Reset-up Duration:   {cfg.get('reset_up_duration')}s\n")
    ...
    def print_summary(self):
        ...
        print(f"  Total time:       {summary['total_session_time']:.0f}s ({summary['total_session_time']/60:.1f} min)")
        ...
        print(f"  Avg time/catch:   {summary['avg_catch_time']:.1f}s (successful)")
